...
/**
* @warning As Angle via DirectX about to be used on Windows, we need to use OpenGL ES 3.0,
* but ProjectM 4.x.x is only compatible with OpenGL ES 3.2, so we need to use the older
* version of ProjectM which is compatible with OpenGL 3.0.
*
* For this reason, we have to maintain two versions of the main source file, one for Windows and
* another for all other OS.
*
* @note If Kodi becomes a support about Vulkan on Windows, we can remove this file and use the
* main source file for all OS, as Vulkan is supported on all OS and Angle will support via Vulkan
* the emulation of OpenGL ES 3.2.
*/

#include "Main_WinV3.h"
